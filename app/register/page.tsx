...
export default function RegisterPage() {
  ...
  async function register(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (data.session) {
        setSuccessMessage("Účet byl vytvořen a jste přihlášený.");
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-slate-950 p-6 text-white">
      <div className="w-full max-w-md rounded-3xl border border-slate-800 bg-slate-900 p-8 shadow-2xl">
        <h1 className="text-center text-4xl font-bold text-cyan-400">AEGRIS</h1>
        <p className="mt-3 text-center text-slate-400">Registrace</p>
        ...
        <form onSubmit={register} className="mt-8">
          ...
          <label htmlFor="password" className="mb-2 mt-4 block text-sm font-semibold text-slate-300">
            Heslo
          </label>
          <input
            id="password"
